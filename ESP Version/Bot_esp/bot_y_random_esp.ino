...
#include <EEPROM.h>               // simple KV storage
#include <ESP8266WebServer.h>     // tiny HTTP server
...
//const char* CHAT_ID   = "987654321";  // your personal chat
...
// ---------------------------------------------------------
// Random hourly triggers:
// Every hour pick 4 distinct random minutes (1..60) and fire
// sonSignal + nivel1Signal at those minutes.
const unsigned long HOUR_MS = 3600000UL;  // 1 hour in ms
unsigned long hourStartMillis = 0;

int AUTO_COUNT = 5;       // number of random triggers per hour (e.g., 4)
int autoMinutes[60];      // max possible 60, we will use AUTO_COUNT entries
bool autoFired[60];

void scheduleNewHour() {
  if (AUTO_COUNT < 1) AUTO_COUNT = 1;
  if (AUTO_COUNT > 60) AUTO_COUNT = 60;

  int pool[60];
  for (int i = 0; i < 60; i++) pool[i] = i;

  // Fisher–Yates partial shuffle for AUTO_COUNT values
  for (int i = 0; i < AUTO_COUNT; i++) {
    int j = i + random(60 - i);  // random index
    int tmp = pool[i];
    pool[i] = pool[j];
    pool[j] = tmp;

    autoMinutes[i] = pool[i];
    autoFired[i]   = false;
  }

  Serial.print("New hour schedule (minutes 1..60): ");
  for (int i = 0; i < AUTO_COUNT; i++) {
    Serial.printf("%d ", autoMinutes[i] + 1);
  }
  Serial.println();
}

void handleHourlyAutoTriggers(unsigned long now) {
  if (hourStartMillis == 0) {
    hourStartMillis = now;
    scheduleNewHour();
  }

  if (now - hourStartMillis >= HOUR_MS) {
    hourStartMillis = now;
    scheduleNewHour();
  }

  unsigned long elapsed = now - hourStartMillis;
  int minute = elapsed / 60000UL;
  if (minute < 0 || minute >= 60) return;

  for (int i = 0; i < AUTO_COUNT; i++) {
    if (!autoFired[i] && minute == autoMinutes[i]) {
      autoFired[i] = true;

      Serial.printf("Auto trigger at minute %d (1-based %d)\n",
                    minute, minute + 1);
...
void setup() {
  ...
  randomSeed(analogRead(A0));   // seed RNG for hourly random minutes
  ...
  if (!connectToKnown()) {
    ...
    tgClient.setInsecure();          // Telegram TLS
  }
...
// LOOP
void loop() {
  server.handleClient();             // cheap when not in AP mode
  unsigned long now = millis();

  // Automatic RF triggers at 4 random minutes each hour
  handleHourlyAutoTriggers(now);

  // Poll Telegram every POLL_INTERVAL ms
